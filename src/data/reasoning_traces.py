...
# "openthoughts" is tentatively apache-2.0 (open-thoughts/OpenThoughts-114k's card at plan time);
...
def openthoughts_row_to_messages(row: dict, source: str = "openthoughts") -> Optional[dict]:
    """Map an `open-thoughts/OpenThoughts-114k` row into a tagged trace row, or None to skip.

    OpenThoughts rows are ShareGPT-style: a `conversations` list of `{from, value}` turns
    (`from` in `system`/`human`/`gpt`). Falls back to the MoT `messages` / `problem`-`solution`
    shape (`mot_row_to_messages`) for rows that don't carry `conversations`. Field names are a
    reasonable guess from the dataset's card — verify against the live dataset before the real
    build (not run live in this dispatch)."""
    ...
    if isinstance(convs, (list, tuple)):
        ...
        out: List[dict] = []
        for turn in convs:
            role = role_map.get((turn.get("from") or "").strip().lower())
        ...
        if out and out[0]["role"] in ("system", "user") and out[-1]["role"] == "assistant":
            return _tag(out, source)
        return None
    return mot_row_to_messages(row, source=source)


def load_openthoughts(split: str = "train", max_examples: Optional[int] = None) -> Iterator[dict]:
    """Stream open-thoughts/OpenThoughts-114k and map rows to tagged trace rows (lazy
    `datasets`)."""
    from datasets import load_dataset  # pragma: no cover - network/optional extra

    ds = load_dataset("open-thoughts/OpenThoughts-114k", split=split, streaming=True)
    n = 0
    for row in ds:
...
def load_openthoughts2(split: str = "train", max_examples: Optional[int] = None) -> Iterator[dict]:
    """Stream open-thoughts/OpenThoughts2-1M (~1M rows) and map rows to tagged trace rows (lazy
    `datasets`). OpenThoughts2-1M **supersets** OpenThoughts-114k (augments it with additional
    curated + synthetic math/code traces), so use this *instead of* `openthoughts` to avoid
    duplicate traces (#65 corpus-extension session, 2026-07-04). Same ShareGPT-style
    `conversations` shape (`from`/`value` turns) as OpenThoughts-114k, so this reuses
    `openthoughts_row_to_messages` unchanged."""
    from datasets import load_dataset  # pragma: no cover - network/optional extra

    ds = load_dataset("open-thoughts/OpenThoughts2-1M", split=split, streaming=True)
    n = 0
    for row in ds:
        rec = openthoughts_row_to_messages(row, source="openthoughts2")
        if rec is None:
            continue
        yield rec
        n += 1
...
def load_topup(dataset: str, split: str = "train",
    ...
    `topup`. Traces are re-tokenized under Qwen3, so the generating model is unconstrained."""
    from datasets import load_dataset  # pragma: no cover - network/optional extra
