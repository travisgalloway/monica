...
def build_corpus(records: Iterable[Record], out_uri, *, min_chars: int = 1,
                 quality: bool = False, license_filter: bool = False,
                 drop_minified: bool = False, drop_autogen: bool = False,
                 scrub: bool = False, shard_size_mb: int = 128,
                 stats=None) -> List[str]:
    """The Stage-2/3 flow end to end: normalize -> filter -> write Parquet shards.
    Returns the shard paths written; pass a `filters.FilterStats` as `stats` to collect
    drop/scrub counts. The Stage-3 flags default off (the #69 local gate is unchanged)."""
...
def main() -> None:
    ...
    shards = build_corpus(records, args.out, min_chars=args.min_chars,
                          ...
                          scrub=args.scrub, shard_size_mb=args.shard_size_mb, stats=stats)
    print(f"wrote {len(shards)} shard(s) -> {args.out}  filter={stats.as_dict()}")
