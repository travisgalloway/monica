...
The *pipeline* is already proven end-to-end on cloud — **Path A** ran a real reasoning teacher + real
~1B student through all three distill stages on an A100 (PRs #139, #140). Path B is the same pipeline
at **full scale and seq_len 8192**, retargeted to the new **Qwen3-4B-Thinking-2507** conversion teacher
(the switch is recorded on #65). All code is merged: precompute (#135/#94), distill driver (#137/#81),
...
- **Path A validated** (2026-06-21, A100 80GB, ~1hr, pod terminated): real reasoning teacher + real
  ~1B student, all three stages' losses dropped (mixing-match 0.00114→0.00025, hidden-align 186.9→49.6,
  ...
  (no 8k corpus built yet). Manifest `config/manifests/validation/student-1b-attn-hi-1k.yaml` (PR #140).
  **Note:** Path A predates the teacher switch — it used the prior 7B OpenR1 teacher on the Qwen2.5
  view, so its memory/throughput numbers below are the *7B* envelope; the Qwen3-4B teacher is lighter.
- **On R2** — `s3://monica-training/`:
  ...
  - `reserve-pretrain/tokenized/v1-qwen25-1k/` — the 1.91B-token **seq_len 1024** Qwen2.5 view (built
    for the poc-qwen run and reused by Path A). Qwen3 is token-aligned with Qwen2.5, but Path B needs a
    fresh **`qwen3`** view at **seq_len 8192** (Step 1) — this 1k Qwen2.5 view is neither.
...
- **Manifests** — `student-1b-attn-{hi,lo}.yaml` (the seq_len **8192** sweep, ~1.03B each) point at
  the **same** `poc-distill/corpus/tokenized/qwen3-8k` + `poc-distill/teacher-outputs/topk-logits`.
- **Pod ops dir** — `~/.claude/monica-runpod-ops/` (holds the prior pod's TERMINATED state).

## Decisions locked

- **Teacher fixed**: `Qwen/Qwen3-4B-Thinking-2507` (Apache-2.0, thinking-mode CoT). Changing it
  invalidates the teacher outputs — so it is pinned first (#91). Qwen3 ≠ Qwen2: the teacher forward
  applies per-head RMSNorm to Q/K before RoPE, has no QKV bias, and ties embeddings (handled in the
  backends — no run-time concern).
- **Tokenizer**: Qwen3 unified BPE, vocab **151,669** → **uint32** packing (#90). Shared with the
  teacher; token-aligned with Qwen2.5 so logit KD stays aligned, with native `<think>`/`</think>` ids.
...
```
  cleaned JSONL ──tokenize@8k──▶ corpus/tokenized/qwen3-8k ──┐         (frozen, built once)
                                                             ├─▶ 4B teacher precompute ─▶ teacher-outputs/topk-logits
                                                     ...
                                                     └─────────┬───────────┘   └──────────┬──────────┘
                                                               └──── pick winner ─────────┘
                                                                          │
                                  shared/sft/qwen3-8k ───▶ instruct-sft ─▶ reasoning-sft ─▶ grpo ─▶ eval
```
...
# main backend venv (training / precompute / data) — install the cuda-fast extra so the fused Mamba
# Triton scan + causal-conv1d kernels (#40) are present; without them the SSD scan falls back to
# pure PyTorch and a sweep silently runs slow (the backend logs a warning at startup if they're absent):
...
#   HF_TOKEN is OPTIONAL here: Qwen3-4B-Thinking-2507 is Apache-2.0 and openly downloadable (unlike
#   the prior gated 7B teacher). Set it only if anonymous HF downloads hit a rate limit.
```
...
re-tokenize it to the Qwen3 vocab at seq_len 8192** — no re-clean, no datatrove. (The existing
`v1-qwen25-1k` view is the wrong tokenizer *and* the wrong seq_len, so it can't be reused directly.)

```bash
# A. (recommended) re-tokenize the existing cleaned JSONL to the 8k Qwen3 view:
python -m src.data.r2_sync down s3://monica-training/reserve-pretrain/cleaned /vol/cleaned
python -m src.data.shard --in /vol/cleaned --out /vol/corpus/qwen3-8k \
    --tokenizer qwen3 --seq-len 8192 --shard-size-mb 512
python -m src.data.r2_sync up /vol/corpus/qwen3-8k \
    s3://monica-training/poc-distill/corpus/tokenized/qwen3-8k
...
python -m src.data.split --shards /vol/corpus/qwen3-8k --out /vol/split8k --val-tokens 10000000
...
python -m src.data.instruct_sft --sources oasst1 flan --tokenizer qwen3 \
    --seq-len 8192 --out-root /vol/shared
#   -> /vol/shared/sft/{cleaned/instruct/records.jsonl, tokenized/qwen3-8k/instruct.jsonl}

# reasoning traces (<think>…</think><answer>…</answer>):
python -m src.data.reasoning_sft --sources mot --tokenizer qwen3 \
    --seq-len 8192 --chunk-align 64 --out-root /vol/shared
#   -> /vol/shared/sft/tokenized/qwen3-8k/{reasoning.jsonl, reasoning-packed/}
...
- Path A measured the *7B* forward at ~7 s/batch (seq **1024**, batch 8) on an A100; the Qwen3-4B
  teacher is smaller, so expect it lighter — but at seq 8192 the batch must shrink anyway, so **bench
...
# degradation natively (knob OFF). Use --batch-size 2: at seq 8192 the (B,T,V=151669) logit tensor
...
python scripts/eval_olmes.py --config <winner config> --weights <winner weights> \
    --tokenizer qwen3 --tasks hellaswag,arc_easy,arc_challenge,piqa --limit 200 \
...
|---|---|---|---|
| 1 corpus tokenize@8k | CPU | re-tokenize cleaned JSONL to Qwen3 | `poc-distill/corpus/tokenized/qwen3-8k` |
| 2 SFT corpus | CPU | instruct + reasoning, ChatML | `shared/sft/tokenized/qwen3-8k` |
| 3 teacher precompute | **GPU (A100/H100 80GB)** | **dominant \$** — 4B fwd over whole corpus, once | `poc-distill/teacher-outputs/topk-logits` |
...
- **HF_TOKEN optional** for the Qwen3-4B teacher (Apache-2.0, ungated) — set it only on rate limits.
  This differs from the prior 7B teacher, which was gated and required it.
