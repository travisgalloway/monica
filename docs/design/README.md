...
13. **[The M12 code model — Mamba-2 hybrid MoE + SSI](13-code-model-moe.md)** — the **live
    ...
    **done** → CUDA MoE backend, #214 **done** → FIM/curriculum/evals, #215/#216/#221 **done** →
    ablation sweep #219 → small full run #222 → sparse-upcycled large run #223, **blocked on #252
    (corpus) and #288**) and the secondary SSI structural-signal fold (#225/#226/#227/#230).
...
> pretraining in 08 remains a production reserve (#75). Topic 14 is the **parallel M13 track**
> (#163) — the native Swift/MLX engine — which runs alongside M12 rather than gating it.
...
| Tokenizer (M12) | **own byte-level BPE — native Swift** (`swift/Sources/MonicaTokenizer`, #191/#245) | cross-platform (macOS + Linux/CUDA), bit-identical; own tiktoken-style format; o200k-style pretokenizer (digit-split ≤3, indentation merge); uint16; **MLX deliberately not used** (BPE is CPU/integer work). Python code-path retired | `docs/design/13-code-model-moe.md` (MHM-P1b) |
| Model sizes (M12) | small ~120M-act/700M-tot; large "Large A" ~700M-act/3.5B-tot — **both at `d_model 768`** | large is **sparse-upcycled** from the small dense ckpt (#200); ablation sweep picks the layout (#219). **Resolved 2026-08-09 (#272):** Large A was re-shaped from `d_model 1536` to 768 (56 layers, `moe_every 3`, 64 experts top-8, `moe_d_ff = d_inner` → 3.88B/710M) so a single dense run seeds both rungs. **#271** (FSDP/expert-parallel) is **built** (`src/model/cuda_distributed.py`, `src/train/parallel.py`, `scripts/train_dist.py`); #288 — grad_checkpoint not composing with FSDP2 — is what now blocks the large run | `docs/design/13-code-model-moe.md` |
